...
def run(protocol: protocol_api.ProtocolContext):
    ...
    flowrate(pipette,20)
    pipette.pick_up_tip()
    pipette.transfer(5,tuberack["A3"].bottom(70),wellplate["H12"],new_tip="never") #setup starter
    out(pipette,blowOut=False)
    pipette.drop_tip()
    ...
    template=KineticsTemplate(protocol,pipette,sources=sources,washers=[-1,2],deposits=[(1,5,0),(-1,5,0),(-2,5,0),(2,5,0),(3,5,0)],volumes={'master':25,'starter':5,'killer':5,'washer':50},mixing={'starter':Mixing(2,10,blowOut=False,offset=1),'deposit':Mixing(0,5,blowOut=False,offset=1),'washer':Mixing(2,12,blowOut=False,offset=1)})
...
class Mixing:
    def __init__(self,reps,vol,offset=0,overDispense=2,blowOut=True):
        self.reps,self.vol,self.offset,self.oDisp,self.blowOut=reps,vol,offset,overDispense,blowOut
    def use(self,pipette,position=None):
        oa,od=pipette.well_bottom_clearance.aspirate,pipette.well_bottom_clearance.dispense
        pipette.well_bottom_clearance.aspirate,pipette.well_bottom_clearance.dispense=self.offset,self.offset
        if position:
            position=loc(position)
            p=position.point
            position.move(types.Point(p.x,p.y,p.z+self.offset))
        if self.reps!=0 and self.vol!=0:
            pipette.mix(self.reps,self.vol,position)
        out(pipette,None,self.oDisp,self.blowOut)
        pipette.well_bottom_clearance.aspirate,pipette.well_bottom_clearance.dispense=oa,od
...
class KineticsLayout: #a class for organizing well functions in an experiment instance
    ...
    def pathL(self,path): #count the length of a given path
        ...
        return l
    def nearestWasher(self,pos): #get the index of the nearest washer, s is selected washer
        s,l=None,inf
