...
A simple protocol for testing out different flow rates along with the blowout feature
Flowrates (by column): default (3.78 ul/s), 5 ul/s, 7 ul/s, 10 ul/s, 15 ul/s, 20 ul/s
Row A is without blowout, row B is with blowout'''
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    pipette.pick_up_tip()
    pipette.distribute(5,tuberack["A3"].bottom(70),[wellplate["A"+str(i+1)].bottom(5) for i in range(len(rates))]+[wellplate["B"+str(i+1)].bottom(5) for i in range(len(rates))],new_tip='never')
    pipette.drop_tip()
    ...
    for r in rates:
        ...
        pipette.dispense(7,wellplate["A"+str(rates.index(r)+1)].bottom(5),rate=r)
        pipette.drop_tip()
        pipette.pick_up_tip()
        pipette.aspirate(5,tuberack["A3"].bottom(70),rate=r)
        pipette.dispense(5,wellplate["B"+str(rates.index(r)+1)].bottom(5),rate=r)
        pipette.blow_out()
        pipette.drop_tip()